...
#
# usage: scripts/eval/run_leaderboard_official.sh <label> <arch> <ckpt> <routes_xml> [routes_subset] [port] [tm_port]
#
...
AGENT="${EVAL_AGENT:-$MTHESIS/scripts/eval/bench2drive_agent.py}"
...
if [ ! -x "$EVAL_PYTHON" ]; then
  ...
  echo "For TransFuser++, build its environment first: bash scripts/setup/setup_tfpp_env.sh"
  exit 1
fi

if [ "$AGENT" = "$MTHESIS/scripts/eval/bench2drive_agent.py" ]; then
...
(
  while sleep 60; do
    "$EVAL_PYTHON" "$MTHESIS/scripts/eval/check_leaderboard_results.py" \
...
"$EVAL_PYTHON" "$MTHESIS/scripts/eval/check_leaderboard_results.py" "$OUT/${LABEL}.json"
VALID=$?
if [ $VALID -ne 0 ]; then
