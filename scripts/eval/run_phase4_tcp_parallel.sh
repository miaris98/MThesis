...
# not once per chunk.
#
# usage: scripts/eval/run_phase4_tcp_parallel.sh <chunk1_ids_csv> <chunk2_ids_csv> <chunk3_ids_csv>
set -x
LABEL="tcp_full51"
...
bash /workspace/MThesis/scripts/eval/run_phase4_chunk.sh "$LABEL" tcp "$CKPT" - 2020 8020 0 "$1" &
PID1=$!
bash /workspace/MThesis/scripts/eval/run_phase4_chunk.sh "$LABEL" tcp "$CKPT" - 2000 8000 0 "$2" &
PID2=$!
bash /workspace/MThesis/scripts/eval/run_phase4_chunk.sh "$LABEL" tcp "$CKPT" - 2010 8010 0 "$3" &
PID3=$!
