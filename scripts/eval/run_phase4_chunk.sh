...
#
# usage: scripts/eval/run_phase4_chunk.sh <label> wor <arch> <ckpt> <port> <tm_port> <gpu_rank> <ids_csv>
#        scripts/eval/run_phase4_chunk.sh <label> tcp <ckpt> -      <port> <tm_port> <gpu_rank> <ids_csv>
set -x
...
for rid in "${ROUTE_ARR[@]}"; do
  ...
  sleep 2
  if [ "$KIND" = "tcp" ]; then
    TCP_SKIP_SAVE=1 timeout "$PER_ROUTE_TIMEOUT" bash "$MTHESIS/scripts/eval/run_tcp.sh" "${LABEL}_perroute/${rid}" "$ARG3" "$ROUTES" "$PORT" "$TM_PORT" "$GPU_RANK" "$rid" \
      >> "/workspace/phase4_${LABEL}_chunk_${PORT}.log" 2>&1
  else
    timeout "$PER_ROUTE_TIMEOUT" bash "$MTHESIS/scripts/eval/run_bench2drive.sh" "${LABEL}_perroute/${rid}" "$ARG3" "$ARG4" "$ROUTES" "$PORT" "$TM_PORT" "$GPU_RANK" "$rid" \
      >> "/workspace/phase4_${LABEL}_chunk_${PORT}.log" 2>&1
  fi
