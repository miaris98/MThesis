...
#
# usage: scripts/eval/run_closed_loop_arms_by_seed.sh <training_seed> [port_base]
#
...
set -x
TRAIN_SEED="${1:?usage: scripts/eval/run_closed_loop_arms_by_seed.sh <training_seed> [port_base]}"
PORT_BASE="${2:-2000}"
# The repository root, two levels up: it is what puts `src` on the import path and
# what the output paths below are relative to.
...
run_arm () {
  ...
  bash "$(dirname "${BASH_SOURCE[0]}")/../setup/start_carla_multi.sh" "$port" || return 1
  CARLA_ROOT=/workspace/carla /workspace/venv_carla/bin/python scripts/eval/eval_wor_closed_loop.py \
