...
#
# usage: scripts/eval/run_fast_eval.sh <label> <arch> <ckpt> [preset] [town] [port]
...
LABEL="${1:?usage: scripts/eval/run_fast_eval.sh <label> <arch> <ckpt> [preset] [town] [port]}"
...
START=$(date +%s)
"$PY" "$MTHESIS/scripts/eval/eval_wor_closed_loop.py" \
  --checkpoint="$CKPT" \
  --policy_arch="$ARCH" \
