...
# Everything below is addressed from the repository root (two levels up), which is also
# what puts `src` on the default import path for the python it launches.
...
if [ ! -d "$DATA_DIR" ]; then
    ...
    log "Run scripts/setup/setup_vastai.sh first, or set DATA_DIR. Training would otherwise fall"
...
run_one() {
    ...
    "$PYTHON_BIN" scripts/training/train_wor.py \
        --data_dir "$DATA_DIR" \
        --save_dir "$dir" \
        --run_label "$label" \
        --seed "$seed" \
...
log ""
"$PYTHON_BIN" scripts/analysis/compare_wor_runs.py --glob "$OUT_ROOT/*" \
