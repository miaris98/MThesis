...
Usage:
    python scripts/training/train_wor.py --data_dir dataset/ --epochs 50 --batch_size 32 --backbone resnet34 --pretrained 1
"""
import argparse
import json
import os
...
# run directly (`python scripts/training/train_wor.py`) or imported as `scripts.training.train_wor`.
...
def parse_args():
    ...
    parser.add_argument("--use_augmented_camera", type=int, default=0, help="Add PDM-Lite's rgb_augmented/ recovery frames as extra training-only samples (1=True, 0=False). Each is a second camera genuinely re-rendered at a random per-route lateral+yaw offset from the true ego pose, with waypoint/route targets re-projected into that offset frame - the corrective 'how to get back onto the route' signal pure behaviour cloning otherwise never sees (closed-loop failures showed as outside_route_lanes + vehicle_blocked with no recovery; see struggle-solutions.md S-020/S-021). No effect on validation or at inference - it only adds training samples, and requires the dataset to actually contain rgb_augmented/ (confirmed present in the released autonomousvision/PDM_Lite_Carla_LB2 archives)")
