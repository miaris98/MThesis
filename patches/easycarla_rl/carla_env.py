...
Patched by this repo (see setup_vastai.sh) to add opt-in shared_mode support: when
params['shared_mode'] is True, this instance spawns and drives its own ego vehicle
within a CARLA world/client owned by an external SharedServerCarlaVectorEnv
coordinator, instead of connecting to its own dedicated CARLA server process. This is
what lets N vehicle-envs share ONE CARLA server on single-GPU machines instead of
each spawning a separate server process (which saturates a single GPU's rasterizer
well before raw compute is the bottleneck). shared_mode defaults to False, so every
non-shared code path below is unchanged from upstream.
"""
...
class CarlaEnv(gym.Env):
    def __init__(self, params):
        ...
        # In shared_mode (single-GPU, multi-vehicle-actor architecture), the coordinator
        ...
        if self.shared_mode:
            self.world = params['external_world']
        else:
        ...
        # Get vehicle spawn points: a coordinator-assigned non-overlapping subset in shared
        # mode (so N vehicle-envs sharing one world don't contend for the same points), or
        # the full map list otherwise.
        spawn_point_subset = params.get('spawn_point_subset')
        self.vehicle_spawn_points = list(spawn_point_subset) if spawn_point_subset else list(self.world.get_map().get_spawn_points())
    ...
    def reset(self):
        ...
        if self.shared_mode:
            # Only destroy actors this instance itself spawned - a global type-filter
            # destroy would nuke every other vehicle-env's ego/cameras/sensors sharing
            # this world.
            self._clear_owned_actors()
        else:
        ...
        if not self.shared_mode:
            # In shared mode this is a coordinator-level, one-time setup (traffic lights
            # are global world state) - redoing it on every slot's every episode reset
            # (every ~15-25 steps) would reintroduce the redundant per-reset RPC overhead
            # this whole redesign is meant to eliminate.
            if self.traffic == 'off':
        ...
        self._owned_actor_ids.add(self.collision_sensor.id)
    ...
    def _apply_action(self, action):
        """Apply control to the ego vehicle. Does not tick - caller (self.step() in
        non-shared mode, or the shared-server coordinator) owns the world.tick() call."""
    ...
    def _post_tick(self):
        """Read back state after a world.tick() has already happened and compute
        obs/reward/cost/done. Split out of step() so a shared-server coordinator can call
        world.tick() once for all vehicle-envs and then call this on each of them."""
        if not self.shared_mode:
            # No single sensible spectator transform with N egos sharing a world, and
            # dead weight under -RenderOffScreen anyway.
    ...
    def _set_synchronous_mode(self, synchronous=True):
        ...
        """
        if self.shared_mode:
            # Only the shared-server coordinator may own sync-mode/tick for a shared
            # world - a stray call here (e.g. via the recursive reset() retry path)
            # must not touch it.
            return
    ...
    def _try_spawn_random_vehicle_at(self, transform, number_of_wheels=[4]):
        ...
        if vehicle is not None:
            self._owned_actor_ids.add(vehicle.id)
    ...
    def _try_spawn_random_walker_at(self, transform):
        ...
        if walker_actor is not None:
            self._owned_actor_ids.add(walker_actor.id)
            # Spawn a controller for the walker
            walker_controller_bp = self.world.get_blueprint_library().find('controller.ai.walker')
            walker_controller_actor = self.world.spawn_actor(walker_controller_bp, carla.Transform(), walker_actor)
            self._owned_actor_ids.add(walker_controller_actor.id)
...
# =============================== Visualize current reference waypoints ===============================
    ...
    def _clear_owned_actors(self):
        """Destroy only actors this instance itself spawned (tracked in
        self._owned_actor_ids), never a global type-filter. Used instead of
        _clear_all_actors in shared_mode, where a global filter would destroy every
        other vehicle-env's ego/sensors sharing this world."""
        if not self._owned_actor_ids:
            return
        owned_ids = list(self._owned_actor_ids)
        ...
        batch = [carla.command.DestroyActor(actor_id) for actor_id in owned_ids
                 if self.world.get_actor(actor_id) is not None]
        if batch and self.client is not None:
            try:
