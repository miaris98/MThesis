"""Unit tests for RolloutBuffer and Generalized Advantage Estimation (GAE)."""
import unittest
import torch
import numpy as np
...
class TestRolloutBuffer(unittest.TestCase):
    """Test suite verifying RolloutBuffer storage, GAE calculation, and minibatch generation."""
    ...
    def test_rollout_buffer_gae(self):
        ...
        buffer = RolloutBuffer(buffer_size=buffer_size, gamma=0.99, gae_lambda=0.95, device=device)

        for i in range(buffer_size):
            buffer.add(
                obs_vis=torch.randn(128),
