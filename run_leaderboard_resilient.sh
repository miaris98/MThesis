...
#
# usage: run_leaderboard_resilient.sh <label> <arch> <ckpt> <routes_xml> [subset] [port] [tm_port] [max_attempts]
#
...
CHECK="$MTHESIS/check_leaderboard_results.py"
ATTEMPT_FILES=()
attempt=1
subset="$SUBSET"
prev_retry=""
...
while [ "$attempt" -le "$MAX_ATTEMPTS" ]; do
  ...
  "$MTHESIS/run_leaderboard_official.sh" \
...
$PY "$MTHESIS/merge_leaderboard_results.py" \
