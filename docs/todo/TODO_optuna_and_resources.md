...
trial only. Add `optuna/` to the `scripts/sync/sync_experiments.py` include groups.
...
Reuses `WorldOnRailsTrainer` directly rather than shelling out to `scripts/training/train_wor.py`, so pruning
