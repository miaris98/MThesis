...
Reuses the current CARLA 0.9.15 install and `scripts/eval/eval_wor_closed_loop.py`'s existing agent loop.
...
**Done:**
- `scripts/eval/eval_wor_closed_loop.py`: added `load_official_routes(xml_path, town, num_routes, seed)`,
...
`scripts/eval/run_closed_loop_official.sh`, 3 concurrent CARLA servers (ports 2000/2010/2020), same
...
Paired bootstrap (`scripts/eval/compare_closed_loop.py`, `driving_score`, 10 shared routes) — **all three
