...
- [x] Built `scripts/analysis/check_val_noise.py`: route-level bootstrap, paired model-vs-model comparison,
      train/held-out gap.
...
- [x] Found the 5.45× memorisation gap — reversing 13.2's "underfitting" for the fixed
      config. (13.18)
- [x] `scripts/setup/download_pdm_lite.py`: disk-aware, smallest-first, extract-and-delete, resume-safe.
...
- [x] Built closed-loop CARLA evaluation end to end and verified it against a live server:
      ...
      unit tests), `scripts/eval/eval_wor_closed_loop.py` (deterministic rollout — seeded routes AND
      seeded traffic, so checkpoints are comparable route-by-route), `scripts/eval/compare_closed_loop.py`
      ...
      (`docs/guides/carla_vulkan_driver_troubleshooting_guide.md`, 1.6a).
...
| Tool | Purpose |
|---|---|
| `scripts/analysis/check_val_noise.py` | Route bootstrap, paired comparison, train/held-out gap |
| `scripts/setup/download_pdm_lite.py` | Disk-aware dataset download |
| `scripts/analysis/compare_wor_runs.py` | Multi-run table with across-seed spread |
| `scripts/training/run_wor_sweep.sh` | Sequential resumable sweep |
| `scripts/sync/sync_experiments.py` | tar-over-ssh pull with live progress |
