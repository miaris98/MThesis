...
Launched by `scripts/training/launch_retrain_v2.sh`. 50 epochs each, `--batch_size 32` (pinned, do not change).
...
- `scripts/training/build_feature_cache.py` — NEW. One-time pass, resumable, atomic temp+rename writes.
...
- `src/training/wor_eval.py` — new `to_device_batch_with_features()` (the old 6-tuple
  `to_device_batch` is left untouched for `scripts/analysis/check_val_noise.py`).
...
- Stale `256x256` assumptions in `scripts/training/train_wor.py`: banner printed a feature grid the run never used
...
- After training: re-run the 112-route official Leaderboard eval + the 38-route Bench2Drive pass.
  Harness exists: `scripts/eval/run_leaderboard_official.sh`, `scripts/eval/run_leaderboard_resilient.sh`,
  `scripts/eval/check_leaderboard_results.py`, `scripts/eval/merge_leaderboard_results.py`, `scripts/eval/scenario_breakdown.py`.
...
> Read `docs/design/continuation_prompt.md` (committed at 2d1078b), check both training arms are still alive,
