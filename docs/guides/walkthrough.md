...
We have updated [start_dashboard.sh](../../scripts/sync/start_dashboard.sh) and [train_rl_agent.py](../../scripts/training/train_rl_agent.py) to restore the classic 3-pane monitoring layout and record hardware telemetry (GPU memory, CPU load, System RAM) in step CSV logs and MLflow dashboard metrics.

---

## 1. Restored 3-Pane `tmux` Dashboard Layout

In [start_dashboard.sh](../../scripts/sync/start_dashboard.sh):
...
* **Single MLflow UI Daemon**: `scripts/training/train_rl_agent.py` manages background MLflow server auto-launch without taking up a tmux pane or running duplicate instances.

---

## 2. Hardware Metrics Added to CSV & MLflow

[train_rl_agent.py](../../scripts/training/train_rl_agent.py) now queries real-time hardware telemetry at every step:
...
- **Syntax & Compilation Test:** `scripts/training/train_rl_agent.py` passed with **0 errors**.
