...
`scripts/training/run_multi_carla_training.sh` and the tmux MLflow server already expect them.
...
```bash
python scripts/training/train_wor.py --data_dir dataset/wor_trajectories --epochs 50
...
```bash
python scripts/training/train_wor.py --save_dir "E:/MThesis_EXP/checkpoints/wor_qwen100m"
```
...
python scripts/sync/sync_experiments.py --ssh-cmd "ssh -p 12345 root@ssh5.vast.ai"

# Everything: mlruns, telemetry, TensorBoard events, checkpoints, videos
python scripts/sync/sync_experiments.py --ssh-cmd "ssh -p 12345 root@ssh5.vast.ai" --include all

# Pick and choose
python scripts/sync/sync_experiments.py --ssh-cmd "..." --include mlruns checkpoints
```
...
For the *live* dashboard on a running instance, [tunnel_mlflow.ps1](../../scripts/sync/tunnel_mlflow.ps1)
still does what it always did (SSH port-forward), and the trainer prints a
Cloudflare URL. `scripts/sync/mlflow_local.ps1` is for the permanent archive.
...
[run_multi_carla_training.sh](../../scripts/training/run_multi_carla_training.sh) and
[mlflow_local.ps1](../../scripts/sync/mlflow_local.ps1). Setting it by hand:
...
```bash
python scripts/sync/sync_experiments.py --relink --dest "F:/MThesis_EXP"
```
...
| Script | Purpose |
|--------|---------|
| [sync_experiments.py](../../scripts/sync/sync_experiments.py) | Pull + merge vast.ai runs into the archive |
| [mlflow_local.ps1](../../scripts/sync/mlflow_local.ps1) | Serve the local archive |
| [src/config/paths.py](../../src/config/paths.py) | Resolve the experiment root |
| [download_from_vastai.py](../../scripts/sync/download_from_vastai.py) | Older per-file `scp` downloader; now defaults to the experiment root. `scripts/sync/sync_experiments.py` supersedes it for anything MLflow-shaped |
| [tunnel_mlflow.ps1](../../scripts/sync/tunnel_mlflow.ps1) | SSH-forward the *live* dashboard off an instance |
| [start_mlflow_tunnel.sh](../../scripts/sync/start_mlflow_tunnel.sh) | Cloudflare tunnel for the live dashboard (runs on the instance) |
