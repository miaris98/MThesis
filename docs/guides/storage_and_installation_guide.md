...
- **Cloud Deployment (Vast.ai)**: For training on cloud GPU instances (Vast.ai), see the dedicated [vastai_setup_guide.md](vastai_setup_guide.md).
...
## 5. Automatic Detection in `scripts/carla/carla_runner.py`
`scripts/carla/carla_runner.py` automatically scans drives `C:`, `D:`, `E:`, `F:`, and `G:` under `<Drive>:\Carla` or `<Drive>:\carla`.
