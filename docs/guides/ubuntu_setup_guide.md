...
2. **Set Environment Variable & Launch:**
   ...
   python scripts/carla/carla_runner.py --headless --graphics vulkan --port 2000
   ```
...
- **Evaluation Video**: Run `python scripts/eval/record_eval_video.py --checkpoint checkpoints/ppo_carla_best.pth --steps 300` to generate driving videos with telemetry overlays.
- **Connection Test**: Run `python scripts/carla/test_connection.py --port 2000` anytime to verify server connection.
