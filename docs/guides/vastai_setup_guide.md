...
bash scripts/setup/setup_vastai.sh
```
...
> **Note**: You do not need to manually start CARLA servers when using `scripts/training/run_multi_carla_training.sh`. The training supervisor automatically launches, monitors, and isolates parallel CARLA server instances across ports `2000, 2004, ...`.
>
...
source /opt/conda/bin/activate carla_py38
cd /workspace/MThesis
python scripts/carla/test_connection.py --host 127.0.0.1 --port 2000
```
...
When you launch `scripts/training/run_multi_carla_training.sh`, a public Cloudflare tunnel is automatically started and the live HTTPS dashboard link is printed directly in **bright green** in the terminal:
```text
==============================================================
...
> 💡 **Troubleshooting Guide**: If you experience Error 1033, DNS propagation delays, or tunnel disconnects, refer to the [Cloudflare Tunnel & MLflow Troubleshooting Guide](cloudflare_tunnel_troubleshooting_guide.md) for 1-command diagnosis and fixes.
...
### A. Parallel Multi-CARLA Training Supervisor (`scripts/training/run_multi_carla_training.sh`)
...
bash scripts/training/run_multi_carla_training.sh 2 70000 qwen100m lav Town10HD_Opt --fresh
```

To launch **4 Parallel CARLA Servers** (Ports `2000`, `2004`, `2008`, `2012`) on a 24GB+ GPU:
```bash
bash scripts/training/run_multi_carla_training.sh 4 70000 qwen100m lav Town10HD_Opt --fresh
```

To **resume training** from the latest checkpoint without restarting progress:
```bash
bash scripts/training/run_multi_carla_training.sh 2 70000 qwen100m lav Town10HD_Opt
```
...
python scripts/training/train_rl_agent.py \
...
```bash
python scripts/eval/record_eval_video.py \
